...
class Fitness:
	...
	def __str__(self):
		return "{} mowed with {} instructions, {} steps".format(self.TotalMowed, self.TotalInstructions, self.StepCount)
...
class Repeat:
	...
	def __str__(self):
		return "repeat({}, {})".format(' '.join(map(str, self.Ops)) if len(self.Ops) > 0 else self.OpCount, self.Times)
...
class Func:
	...
	def __str__(self):
		return "func{1}: {0}".format(' '.join(map(str, self.Ops)), self.Id if self.Id is not None else '')
		...
		for index in reversed(range(len(temp))):
			...
			if type(temp[index]) is Func:
				...
				if temp[index].ExpectCall:
					func.Id = len(funcs)
				func.Ops = [i for i in temp[start:end] if type(i) is not Repeat or type(i) is Repeat and len(i.Ops) > 0]
				funcs.append(func)
				del temp[index:end]
...
class LawnmowerTests(unittest.TestCase):
	...
	def test_mow_turn(self):
		...
		def fnCreateField():
			return lawnmower.TroidField(width, height, lawnmower.FieldContent.Grass)
	...
	def test_mow_turn_jump(self):
		...
		def fnCreate():
			return lawnmower.TroidField(width, height, lawnmower.FieldContents.Grass)
	...
	def test_mow_turn_jump_validating(self):
		...
		self.run_with(geneSet, width, height, minGenes, maxGenes, expectedNumberOfInstructions, maxMutationRounds, fnCreateField)
	"""
	...
	def test_mow_turn_jump_func(self):
		width = height = 8
		geneSet = [lambda: Mow(), lambda: Turn(),
		lambda: Jump(random.randint(0, min(width, height)), random.randint(0, min(width, height))), lambda: Func()]
		minGenes = 3
		maxGenes = 20
		maxMutationRounds = 3
