	...
	def display(self, mower):
		for rowIndex in range(self.Height):
			if rowIndex != mower.Location.Y:
				...
				r[mower.Location.X] = "{}{}".format(FieldContents.Mower, mower.Direction.Symbol)
				row = ' '.join(map(str, r))
			print(row)
...
class Mower:
	...
	def mow(self, field):
		...
		if isValid:
			...
			self.StepCount += 1
			field.set(self.Location, self.StepCount if self.StepCount > 9 else "{} ".format(self.StepCount))
