...
class RegexTests(unittest.TestCase):
	...
	def test_state_codes(self):
		...
		unwanted = {"N" + l for l in "ABCDEFGHIJKLMNOPQRSTUVWXYZ" if "N" + l not in wanted}
		customOperators = [
...
def mutate_to_character_set(genes):
	...
	ors = [i for i in range(1, len(genes) - 1)
			if genes[i] == '|' and genes[i-1] not in allMetas and genes[i+1] not in allMetas]
	if len(ors) == 0:
		return False
	shorter = [i for i in ors if sum(len(w) for w in genes[i-1:i+2]) >
